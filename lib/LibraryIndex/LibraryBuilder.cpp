...
struct StagedEntry {
  ...
  // Series name the book declares, shown on the shelf; the ordering key lives
  // in ClixRecord::seriesKey.
  uint8_t seriesLen;
...
// Reads the reader's finished flag straight out of the book's stats file. The
// "completed" byte sits at offset 11 in every stats version from v2 on (see
...
[[gnu::noinline]] bool stageRecord(WalkState& st, const std::string& name, const uint32_t fileSize,
  ...
  if (reuseMetadata) {
    ...
    if (!st.previous->readSeries(priorRecord, series) && st.previous->ioFailed()) {
      st.failed = true;
      return false;
    }
  ...
  if (!reuseMetadata && extractionExpected) {
    ...
    float bookSeriesIndex = 0.0f;
    if (epub.loadMetadata(bookTitle, author, bookSeries, bookSeriesIndex)) {
      ...
      authorFromBook = !author.empty();
      series = std::move(bookSeries);
      seriesIndex = bookSeriesIndex;
    } else {
  ...
  if (!reuseMetadata) {
    ...
    // Series key: folded name prefix, then a big-endian fixed-point index so one
    // compare orders books by series and then by position within it. No series
    // is 0xFF throughout, which sorts to the end of the shelf.
    if (series.empty()) {
      memset(entry.record.seriesKey, 0xFF, sizeof(entry.record.seriesKey));
    } else {
      memset(entry.record.seriesKey, 0, sizeof(entry.record.seriesKey));
      const std::string seriesFolded = fold(series, true);
      const size_t prefix = std::min<size_t>(CLIX_SERIES_FOLD_BYTES, seriesFolded.size());
      const size_t safe = static_cast<size_t>(utf8SafeTruncateBuffer(seriesFolded.data(), static_cast<int>(prefix)));
      memcpy(entry.record.seriesKey, seriesFolded.data(), std::min(safe, CLIX_SERIES_FOLD_BYTES));
      uint32_t q = seriesIndex > 0.0f ? static_cast<uint32_t>(seriesIndex * 100.0f + 0.5f) : 0u;
      if (q > 0xFFFFu) q = 0xFFFFu;
      entry.record.seriesKey[CLIX_SERIES_FOLD_BYTES] = static_cast<char>((q >> 8) & 0xFF);
      entry.record.seriesKey[CLIX_SERIES_FOLD_BYTES + 1] = static_cast<char>(q & 0xFF);
    }
  }
  ...
  entry.seriesLen = static_cast<uint8_t>(std::min(series.size(), STAGE_AUTHOR_BYTES));
...
bool emitIndex(const char* folderStagePath, WalkState& st, const uint16_t* order, const uint16_t* resolvedFirstSeen,
  ...
  // --- series order --------------------------------------------------------
  //
  // The record's 12-byte seriesKey already packs the folded series name and the
  // book's position within it, so a single bytewise sort yields series order and
  // puts books with no series (0xFF) at the end.
  auto seriesSort = rankable && n > 1 ? makeUniqueNoThrow<SortKey[]>(n) : nullptr;
  if (seriesSort) {
    for (uint16_t i = 0; i < n; i++) {
      serviceBuilder(serviceUnits);
      ClixRecord r{};
      if (!readStageAt(static_cast<uint64_t>(order[i]) * STAGE_STRIDE, &r, sizeof(r))) break;
      memcpy(seriesSort[i].key, r.seriesKey, sizeof(seriesSort[i].key));
      seriesSort[i].ordinal = i;
    }
    if (!ioFailed && n > 1) {
      delay(1);
      std::sort(seriesSort.get(), seriesSort.get() + n, sortKeyLess);
      delay(1);
    }
  } else if (n > 1) {
    stats.ranksDegraded = true;
  ...
  for (uint16_t k = 0; k < n; k++) {
    serviceBuilder(serviceUnits);
    const uint16_t ordinal = seriesSort ? seriesSort[k].ordinal : k;
    put(&ordinal, sizeof(ordinal));
  }
